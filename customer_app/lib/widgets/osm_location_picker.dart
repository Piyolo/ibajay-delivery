...
class OsmLocationPicker extends StatefulWidget {
  ...
  /// When the parent moves the selection externally (e.g. search), the
  /// camera follows. Compare by lat/lng.
  final PickedLocation? target;

  const OsmLocationPicker({
    super.key,
    this.initial = const PickedLocation(
        lat: LocationConstants.townLat, lng: LocationConstants.townLng),
    this.height = 260,
...
class _OsmLocationPickerState extends State<OsmLocationPicker> {
  ...
  /// Where the user last chose a barangay deliberately (suggestion,
  /// dropdown, or target). Near that spot their choice wins over whatever
  /// the geocoder guesses — rural OSM labels are sparse and can name the
  ...
  bool get _distanceOutside =>
      LocationConstants.haversineKm(
            LocationConstants.townLat,
            LocationConstants.townLng,
            _point.latitude,
            _point.longitude,
          ) >
      LocationConstants.serviceRadiusKm;
  ...
  Future<void> _resolve() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Barangay candidates (PH barangays usually land in village/suburb).
        String? areaName;
        for (final key in [
        ...
        String? match = LocationConstants.matchBarangay(areaName ?? '');

        // Cross-check the geocoder's barangay label against our known
        // centers: OSM's rural place nodes are sparse, so taps in one
        // barangay sometimes come back labeled as its neighbor (Aslum
        // used to snap to Agbago because only a river carried its name).
        final nearest = LocationConstants.nearestBarangay(
          _point.latitude,
          _point.longitude,
        );
        if (match != null && nearest.key.isNotEmpty && nearest.key != match) {
          final labeledKm = LocationConstants.haversineKm(
            _point.latitude,
            _point.longitude,
            LocationConstants.barangayCoordinates[match]!.lat,
            LocationConstants.barangayCoordinates[match]!.lng,
          );
          if (nearest.value * 2 < labeledKm && labeledKm > 1.0) {
            match = nearest.key;
          }
        } else if (match == null &&
            nearest.key.isNotEmpty &&
            nearest.value <= 2.0) {
          match = nearest.key;
        }

        // Municipality candidates decide inside/outside authoritatively.
        String? municipality;
        ...
        if (municipality != null && municipality.isNotEmpty) {
          ...
          outside = !(m.contains('ibajay')) &&
              LocationConstants.neighboringMunicipalities.any((n) => m.contains(n));
          // Unknown municipality name: trust the radius.
        ...
        if (mounted) {
          setState(() {
            ...
            // Near where the user manually chose a barangay, their choice
            // sticks; otherwise the (cross-checked) geocode result applies.
            ...
            child: FlutterMap(
              ...
              children: [
                TileLayer(
                  ...
                  userAgentPackageName: 'com.ibajayeats.app',
                ),
                MarkerLayer(
                  markers: [
                    Marker(
